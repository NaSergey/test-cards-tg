export default function LogoImages() {
  return (
    <svg width="122" height="78" viewBox="0 0 122 78" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M70.9995 14.6049L45.3402 48.4138C43.834 50.4042 40.8485 50.4042 39.3423 48.4138L27.1043 32.1952C25.5981 30.2049 22.6126 30.2049 21.1064 32.1952L0.772618 59.0111C-1.11014 61.4856 0.665034 65.0359 3.75814 65.0359H107.444C110.537 65.0359 112.313 61.4856 110.43 59.0111L76.9706 14.6587C75.4643 12.6683 72.4788 12.6683 70.9726 14.6587L70.9995 14.6049Z" fill="url(#paint0_linear_1679_146)"/>
      <path d="M30.6813 21.3021C36.5637 21.3021 41.3323 16.5334 41.3323 10.651C41.3323 4.76863 36.5637 0 30.6813 0C24.7989 0 20.0303 4.76863 20.0303 10.651C20.0303 16.5334 24.7989 21.3021 30.6813 21.3021Z" fill="url(#paint1_linear_1679_146)"/>
      <path d="M105.614 45.3634C114.251 45.3634 121.252 52.3646 121.252 61.0011C121.252 69.6377 114.251 76.6388 105.614 76.6388C96.9778 76.6387 89.9766 69.6376 89.9766 61.0011C89.9767 52.3647 96.9779 45.3635 105.614 45.3634Z" fill="url(#paint2_linear_1679_146)" stroke="#F1F6FD"/>
      <path d="M105.642 52.6525C105.984 52.6525 106.272 52.9399 106.272 53.2823V60.3722H113.361C113.704 60.3722 113.991 60.6588 113.991 61.0011C113.991 61.3435 113.704 61.6309 113.361 61.631H106.272V68.7208C106.272 69.0633 105.984 69.3507 105.642 69.3507C105.299 69.3505 105.013 69.0632 105.013 68.7208V61.631H97.9229C97.5804 61.631 97.293 61.3436 97.293 61.0011C97.2932 60.6588 97.5805 60.3722 97.9229 60.3722H105.013V53.2823C105.013 52.94 105.299 52.6526 105.642 52.6525Z" fill="white" stroke="#F1F6FD"/>
      <defs>
        <linearGradient id="paint0_linear_1679_146" x1="181.152" y1="-5.2396" x2="101.603" y2="137.396" gradientUnits="userSpaceOnUse">
          <stop stopColor="#2BD1FF"/>
          <stop offset="0.509615" stopColor="#34B8FF"/>
          <stop offset="1" stopColor="#229CFD"/>
        </linearGradient>
        <linearGradient id="paint1_linear_1679_146" x1="54.732" y1="-7.5588" x2="16.938" y2="24.0507" gradientUnits="userSpaceOnUse">
          <stop stopColor="#2BD1FF"/>
          <stop offset="0.509615" stopColor="#34B8FF"/>
          <stop offset="1" stopColor="#229CFD"/>
        </linearGradient>
        <linearGradient id="paint2_linear_1679_146" x1="142.055" y1="33.4107" x2="84.7914" y2="81.3039" gradientUnits="userSpaceOnUse">
          <stop stopColor="#2BD1FF"/>
          <stop offset="0.509615" stopColor="#34B8FF"/>
          <stop offset="1" stopColor="#229CFD"/>
        </linearGradient>
      </defs>
    </svg>
  );
}
